...
export default function Header({
  ...
  return (
    <header className="w-full flex flex-col h-auto z-40 select-none sticky top-0">
      ...
      <div className="w-full glass-panel border-x-0 py-3.5 px-4 sm:px-6 md:px-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        {/* LOGO LOCKUP */}
        <div 
          onClick={() => handleTabChange('collections')}
          className="flex items-start gap-3 cursor-pointer select-none group self-start"
        >
          <div className="relative">
            <span className="font-display text-2xl sm:text-3xl font-black tracking-tight uppercase leading-none text-graphite-black group-hover:text-warning-red transition-colors duration-300">
              boldstar
            </span>
            <div className="absolute -top-1.5 -right-4 flex items-center text-[10px] font-mono text-warning-red font-extrabold">
              777
            </div>
          </div>
          <span className="text-warning-red font-display text-lg animate-spin-slow inline-block">★</span>
          
          <div className="hidden xs:flex flex-col text-[10px] font-mono text-steel-gray leading-tight mt-0.5 border-l border-concrete/40 pl-2.5">
            <span className="font-bold text-graphite-black">ARCHIVE SYSTEM</span>
            <span>VER. 2.0.26 // CORE</span>
          </div>
        </div>

        {/* INTERACTIVE COMPONENT NAVIGATION (Frosted Glass Pills) */}
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono font-medium">
          <button
            onClick={() => handleTabChange('collections')}
            className={`px-4 py-2.5 rounded-xl transition-all duration-300 cursor-pointer flex items-center gap-2 ${
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          <button
            ...
            className={`glass-interactive group px-5 py-2.5 rounded-full flex items-center gap-3 text-xs font-mono cursor-pointer transition-all duration-300 ${
              cartCount > 0 
          ...
          >
            ...
            <span className="text-graphite-black font-bold tracking-wider">
              CART_ARCHIVE <span className="text-steel-gray font-semibold">[{cartCount}]</span>
            </span>

            {cartCount > 0 && (
              <span className="glass-inset px-2 py-0.5 rounded text-[10px] bg-signal-yellow font-mono text-graphite-black font-black border border-graphite-black/20">
                ACTIVE
              </span>
            )}
          </button>
          ...
          <div className="w-9 h-9 rounded-full glass-inset hidden sm:flex items-center justify-center text-xs text-steel-gray cursor-help" title="SYS-DIAL-STATUS: ACTIVE">
            ⚙️
          </div>
        </div>
      </div>
